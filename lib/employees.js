 ...
 * @param {string} [oParameters.dateFormat] Formatting for all of the date outputs. See http://momentjs.com/docs/#/displaying/format/
 * @param {Array} [oParameters.startFrom] Starting point relative to current time [{number}amount, {string}type] ([0,'months'])
 * @param {Array} [oParameters.interval] Interval to check between [{number}amount, {string}type] ([0,'days'])
 * @param {Array} [oParameters.columns] Columns to be included in the export
 * @param {Object} [oParameters.columnFormatters] Formatter functions matching column names. Formatters are called with parameters {Employee}this, {moment}date
 * @constructor
 */
...
Employees.prototype.getAges = function (oDate) {
    ...
    _.each(this.employees, (oEmployee) => {
        ...
        if (oAge.isValid() && oEmployee.getEmployment(oDate)) {
            oAge = moment().diff(oAge, "years");
            for (var i in oCount) {
                if (oAge < parseInt(i)) {
                    var iAge=(parseInt(i)-10).toString();
                    iAge in oCount && oCount[iAge]++;
